...
function cc_integer_to_roman( $integer ) {
 ...
 *
 */
function cc_years_shortcode( $atts, $content = null ) {

	$atts = shortcode_atts( array(
		'start' => absint( cc_caboodle( 'years_from', date('Y') ) ),
		'format' => '',
	), $atts, 'cc_caboodle' );
	...
	if ( strtolower( $atts['format'] ) == 'roman' ) {
		$start = cc_integer_to_roman( $start );
		$date = cc_integer_to_roman( $date );		
	}
...
add_shortcode( 'years', 'cc_years_shortcode' );