 ...
 */
function cc_show_ids_value( $column_name, $id ) {
	if ( 'cc_show_ids' == $column_name ) {
		echo esc_attr( $id );
	}
}
...
add_action( 'admin_head', 'cc_show_ids_css' );

/**
 * Actions/Filters for various tables and the css output
 */
function cc_show_ids_add() {

	// Media
	if ( cc_caboodle( 'show_ids_media', '0' ) == '1' ) {
		add_action( 'manage_media_columns', 'cc_show_ids_column' );
		add_filter( 'manage_media_custom_column', 'cc_show_ids_value', 10, 3 );
	}

	// Links & link categories
	if ( cc_caboodle( 'show_ids_links', '0' ) == '1' ) {
		add_action( 'manage_link_custom_column', 'cc_show_ids_value', 10, 2 );
	...
	if ( cc_caboodle( 'show_ids_posts', '0' ) == '1' ) {
		foreach ( get_post_types() as $posttype ) {
			...
			add_filter( "manage_${posttype}_posts_custom_column", 'cc_show_ids_value', 10, 3 );
	...
	if ( cc_caboodle( 'show_ids_comments', '0' ) == '1' ) {
		...
		add_action( 'manage_comments_custom_column', 'cc_show_ids_value', 10, 2 );
		add_filter( 'manage_edit-comments_sortable_columns', 'cc_show_ids_column' );
	}
}
add_action( 'admin_init', 'cc_show_ids_add' );